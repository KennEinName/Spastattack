...
public class TPHereCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            ...
            annehmen.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§a/tphere accept " + player.getName())));
            ablehnen.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§c/tphere deny " + player.getName())));

            annehmen.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/tphere accept " + player.getName()));
            ablehnen.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/tphere deny " + player.getName()));
            ...
            if (to == null) {
                ...
                if (anfrage.containsKey(to)) {
                    if (anfrage.get(to).contains(player)) {
                        ...
                        player.sendMessage(Spastattack.PREFIX + "§aDu hast eine §eTP Here Anfrage an " + to.getName() + "§a gesendet!");
                        to.sendMessage(Spastattack.PREFIX + "§aDu hast eine §eTP Here Anfrage von " + player.getName() + " §a erhalten.");
            ...
            if (args[0].equalsIgnoreCase("accept")) {
                ...
                player.sendMessage(Spastattack.PREFIX + "§cBitte benutze §e/tphere accept [SPIELERNAME] §coder §e/tphere [SPIELERNAME] deny§c!");
            }
        } else {
            player.sendMessage(Spastattack.PREFIX + "§cBitte benutze §e/tphere [NAME]§c!");
        }
